use bevy::{
    core::{Time, Timer},
    prelude::{Entity, IntoSystem, Plugin, Query, Res, ResMut, State, SystemSet, With},
...
impl Plugin for SwitchStatePlugin {
    fn build(&self, app: &mut bevy::prelude::AppBuilder) {
        app.add_startup_system(setup::setup.system())
            ...
            //State A
            .add_system_set(
                SystemSet::on_enter(AppStates::A).with_system(on_stateA_entered.system()),
            )
            .add_system_set(
                SystemSet::on_update(AppStates::A).with_system(on_stateA_process.system()),
            )
            .add_system_set(SystemSet::on_exit(AppStates::A).with_system(on_stateA_exit.system()))
            //State B
            .add_system_set(
                SystemSet::on_enter(AppStates::B).with_system(on_stateB_entered.system()),
            )
            .add_system_set(
                SystemSet::on_update(AppStates::B).with_system(on_stateB_process.system()),
            )
